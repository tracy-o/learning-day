...
So the call follow this pattern:
```
Belfrage -calls-> aws_ex_ray -calls-> xray daemon -calls-> xray API 
```
...
### Sampling Properly
If we only call the library when we wish to sample, it will mean aws_ex_ray won't touch 90% of our requests. When we start tracing a request we already put the sampling information into the struct.
```elixir
...
Why not decide whether to sample ourselves. Reducing calls to aws_ex_ray by 90%. This also means reducing stress on the ETS pool and UDP pool within aws_ex_ray, and means no redundant tracing operations for traces that will never be sent.
...
Although I think this runtime error is valid, we only want to be tracing once per request. If something is wrong, we would want to see it, not suppress it.

### Other Possible Errors
We have already discussed the poolboy timeout. The error that is produced from this isn't actually damaging to the application, its the time it takes to produce the error. A timeout from the pool will mean the response from the aws_ex_ray will return a `:timeout` atom. This will mean the return value of `Store.MonitorSupervisor.start_monitoring(self())` will also be `:timeout`.
...
A solution to this could be that all interaction with the x-ray library could happen in a separate process which accepts casts, so a runtime error from the aws_ex_ray won't touch the request process.

## Summary
To summarise:
  - xray is used to track a distributed applications performance across services
