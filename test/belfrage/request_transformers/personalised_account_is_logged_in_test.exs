defmodule Belfrage.RequestTransformers.PersonalisedAccountIsLoggedInTest do
  ...
  test "redirect to session.bbc.co.uk/session if not authenticated" do
    envelope = %Envelope{
  ...
  test "redirect to session.test.bbc.co.uk/session if user is authenticated but with an invalid session" do
    envelope = %Envelope{
      request: %Envelope.Request{
        scheme: :https,
        host: "www.test.bbc.co.uk",
  ...
  test "redirect to /session if there is no user_session" do
    envelope = %Envelope{
      request: %Envelope.Request{
        scheme: :https,
        host: "www.bbc.co.uk",
  ...
  test "pass if authenticated and has valid session" do
    envelope = %Envelope{
      request: %Envelope.Request{
        scheme: :https,
        host: "www.bbc.co.uk",
