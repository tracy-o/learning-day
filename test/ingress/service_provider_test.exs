defmodule Ingress.ServiceProviderTest do
  ...
  describe "ServiceProvider.service_for/1" do
    test "when origin is http/s it returns the HTTP Service" do
      assert Services.HTTPMock == ServiceProvider.service_for("https://www.bbc.co.uk")
    end

    test "when origin is not http/s it returns the Lambda Service" do
      assert Services.LambdaMock == ServiceProvider.service_for("lambda-presentation-layer")
    end
  end
end
