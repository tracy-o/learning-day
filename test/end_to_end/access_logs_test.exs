defmodule EndToEnd.AccessLogsTest do
  ...
  test "requests are logged" do
    ...
    captured_log =
      capture_log(fn ->
        conn(:get, build_https_request_uri("/200-ok-response?query=querystring"))
    ...
    assert count_words(captured_log, "REDACTED") == 3, "Failed to replace cookie and ssl values from header"
    ...
    refute captured_log =~ "auth-header-value", ~s("authorization" request header value still present in log)
  end
