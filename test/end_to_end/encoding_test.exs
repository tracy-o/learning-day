defmodule ContentEncodingTest do
  ...
  import Test.Support.Helper, only: [assert_gzipped: 2, build_https_request_uri: 1]
  ...
  test "when the client accepts gzip the response should be gzipped" do
    ...
    conn = conn(:get, build_https_request_uri("/proxy-pass")) |> put_req_header("accept-encoding", "gzip, deflate, br")
  ...
  test "when client does not accept gzip the response is not gzipped" do
    ...
    conn = conn(:get, build_https_request_uri("/proxy-pass")) |> put_req_header("accept-encoding", "deflate, br")
    conn = Router.call(conn, [])
  ...
  describe "when no accept-encoding header is sent" do
    ...
    test "the response is not encoded (when response is from origin)" do
      conn = conn(:get, build_https_request_uri("/proxy-pass"))
      conn = Router.call(conn, [])
    ...
    test "the response is not encoded (when response is from cache)" do
      # seed the cache
      conn(:get, build_https_request_uri("/proxy-pass"))
