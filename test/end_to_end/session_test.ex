defmodule EndToEnd.SessionTest do
  ...
  test "when no authorization token is provided", %{lambda_response: lambda_response} do
    ...
    response_conn =
      conn(:get, "/my/session/webcore-platform") |> Map.put(:host, "www.test.bbc.co.uk") |> Router.call([])
  ...
  test "when an invalid authorization token is provided", %{
    ...
    assert {"location",
            "https://session.test.bbc.co.uk/session?ptrt=https%3A%2F%2Fwww.test.bbc.co.uk%2Fmy%2Fsession%2Fwebcore-platform"} in headers
  end

  test "on Live when a valid authorization token is provided", %{
    valid_access_token: access_token,
    lambda_response: lambda_response
  } do
    Belfrage.Clients.LambdaMock
